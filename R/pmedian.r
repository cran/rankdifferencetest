...
#'   5 \tab 2 \tab `n_analytic` \tab Number of observations after removing non-finite values from the original data. \cr
...
pmedian <- function(
  ...
  # Prepare results
  #-----------------------------------------------------------------------------
  lst <- srt_data(x = data, formula = formula, agg_fun = agg_fun)
  ...
  # Prepare return
  #-----------------------------------------------------------------------------
  data_type <- if (is.null(lst$reference_name)) {
    "Observations"
  } else {
    "Paired differences"
  }
...
pmedian2 <- function(
  ...
  # Prepare results
  #-----------------------------------------------------------------------------
  lst <- srt_data(x = x, y = y, x_name = focal_name, y_name = reference_name)
